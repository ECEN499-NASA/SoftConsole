...
void run_uart_test(void) {
    ...
    // PLIC Initialization
    PLIC_init();
    UART_init(&g_core_uart, LVDS_UART_BASE_ADDR, BAUD_VALUE_57600,
        ...
        if (rx_size > 0) {
            ...
            case '1':
                ...
                if (UART_APB_NO_ERROR == err_status) {
                    if (0 == tx_length) {
                        UART_polled_tx_string(&g_uart,
                                (const uint8_t*) "0 Byte Data Write Successful\n\r");
                    } else {
                        UART_polled_tx_string(&g_uart,
                                (const uint8_t*) "Data Write Successful and Data is: ");
                    ...
                    if (UART_APB_PARITY_ERROR == err_status) {
                        UART_polled_tx_string(&g_uart,
                                (const uint8_t*) "Data Write Parity Error!\n\r");
                    } else if (UART_APB_OVERFLOW_ERROR == err_status) {
                        UART_polled_tx_string(&g_uart,
                                (const uint8_t*) "Data Write Overflow Error!\n\r");
                    } else if (UART_APB_FRAMING_ERROR == err_status) {
                        UART_polled_tx_string(&g_uart,
                                (const uint8_t*) "Data Write Framing Error!\n\r");
                    } else {
                        UART_polled_tx_string(&g_uart,
                                (const uint8_t*) "Data Write Unknown Response!\n\r");
                    }
...
static void uart_display_greeting(void) {
    UART_polled_tx_string(&g_uart,
            (const uint8_t*) "\n\r******************************************************************************\n\r");
    UART_polled_tx_string(&g_uart,
            (const uint8_t*) "**************************** Core UART Example ********************************\n\r");
    UART_polled_tx_string(&g_uart,
            (const uint8_t*) "******************************************************************************\n\r");
    UART_polled_tx_string(&g_uart,
            (const uint8_t*) "This example project Demonstrates the use of UART in Different Modes\n\r");
    UART_polled_tx_string(&g_uart,
            (const uint8_t*) "CoreUART0 is configured in Master Mode and CoreUART1 is configured in Slave Mode\n\r");
    UART_polled_tx_string(&g_uart,
            (const uint8_t*) "\n\r------------------------------------------------------------------------------\n\r");
    UART_polled_tx_string(&g_uart,
            (const uint8_t*) "************* UART Modes supported by this example project are ****************\n\r");
...
static void select_mode_uart(void) {
    ...
    UART_polled_tx_string(&g_uart,
            (const uint8_t*) "Press Key '1' to perform MT-SR (Transmit)\n\r");
    UART_polled_tx_string(&g_uart,
            (const uint8_t*) "Press Key '2' to perform MR-ST (Receive)\n\r");
    UART_polled_tx_string(&g_uart,
            (const uint8_t*) "Press Key '3' to EXIT from the Application \n\r");
    UART_polled_tx_string(&g_uart,
            (const uint8_t*) "------------------------------------------------------------------------------\n\r");
}
